...
// 从工作流定义获取节点顺序（按执行顺序）
function getWorkflowPhaseOrder(): string[] {
  if (workflow.value?.nodes && Array.isArray(workflow.value.nodes)) {
    // 从工作流定义读取节点 ID 列表
    // 按边关系排序（拓扑序）
    const nodes = workflow.value.nodes as Array<{ id: string }>
    const edges = workflow.value.edges as Array<{ from: string; to: string }> || []
...
function phaseLabel(id: string): string {
  // 优先从工作流定义读取节点 label
  if (workflow.value?.nodes && Array.isArray(workflow.value.nodes)) {
    const node = workflow.value.nodes.find((n: any) => n.id === id)
