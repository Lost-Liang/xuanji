...
import { ref } from 'vue'
import { api, type TaskListItem, type TaskDetail } from '../api/tasks'

export const tasks = ref<TaskListItem[]>([])
export const currentDetail = ref<TaskDetail | null>(null)
export const loading = ref(false)

// 抽屉状态
export const showChatDrawer = ref(false)
export const showLogDrawer = ref(false)
export const currentExecId = ref<string>('')
export const currentSessionRefId = ref<string | null>(null)
export const currentRateLimitedUntil = ref<string | null>(null)
export const currentStatus = ref<string>('')

export async function loadList(requirementId?: string) {
  loading.value = true
  try {
...
export function openChatDrawer(row: Pick<TaskListItem, 'id' | 'session_ref_id' | 'status' | 'rate_limited_until'>) {
...
export function openLogDrawer(row: TaskListItem) {
  currentExecId.value = row.id
  currentStatus.value = row.status
  currentRateLimitedUntil.value = row.rate_limited_until
