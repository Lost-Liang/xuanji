...
export const decisionStore = {
   ...
   * @param data.phaseInstanceId 关联的阶段实例 ID（可选）
   ...
   * 按 ID 查询决策
   */
  async getById(id: number): Promise<Decision | null> {
   ...
   * 按执行实例 ID 列出所有决策
   */
  async listByExecution(executionId: string): Promise<Decision[]> {
   ...
   * 按任务 ID 列出所有决策
   */
  async listByTask(taskId: string): Promise<Decision[]> {
    return db.decision.findMany({
      where: { taskId },
      orderBy: { createdAt: 'asc' },
    });
   ...
   * 按阶段实例 ID 列出所有决策
   */
  async listByPhaseInstance(phaseInstanceId: string): Promise<Decision[]> {
    return db.decision.findMany({
      where: { phaseInstanceId },
      orderBy: { createdAt: 'asc' },
    });
  },
   ...
   * 按严重级别过滤（用于仪表盘告警）
   */
  async listBySeverity(executionId: string, severity: string): Promise<Decision[]> {
    return db.decision.findMany({
      where: { executionId, severity },
