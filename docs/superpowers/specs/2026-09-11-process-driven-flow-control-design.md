...
**日期**: 2026-09-11
**状态**: 待评审
**范围**: 流程感知 / 判定 / 决策 / 约束四职责的归属重构 + 调度层参与流程治理
...
`develop` 不在表里 → `loop_counters['test']` **永远是 0** → `done < limit` 恒真 → `loop_max: 2` 形同虚设。**这是空转的直接物理成因，比根因一更致命。**

### 1.5 根因三：`attempt` 与 `session_id` 脱钩，回环退化成「复述」（记忆断线）
...
因为 §1.4 里 `loop_counters['develop']` 恒为 0，`iteration` 恒为 0，于是**每次都命中第 1 行 `attempt=1` 的 phase_instance，每次都拿到同一个 `session_id`，每次都以"续话"方式调用 CLI agent**。
...
> **这条是本设计的第一优先级。** 只修条件函数的 `source`（根因一），`test_pass` 会恢复正常，但只要流程再触发任何一次回环，`resume` 依然会把 agent 锁死在旧会话上，空转换一种形式重演。
...
| | 流程职责 | 现状实现位置 | 状态 |
|---|---|---|---|
| ① 感知 | 读到当前阶段 AI 的产出 | `default-conditions.mts` 里硬编码的节点名字符串 | ❌ 断 |
| ② 判定 | 判断产出是否达标 | `agent-node.mts` 里 AI 自报的 `ok` 布尔 | ⚠️ 无验证 |
| ③ 决策 | 决定前进 / 回退 / 终止 | YAML `edges` | ✅ 有（但输入恒假 → 退化成常量） |
| ④ 约束 | 决定最多几次、何时必须停 | `builder.mts` 的 `FIX_LOOP_KEY` 手写映射表 | ❌ 断 |
| ⑤ 记忆 | 决定"再次进入该节点"时是重做还是续话 | `agent-node.mts` 的 `resume`，与 ④ 无关联 | ❌ 断 |

**YAML 描述了一张流程图，但流程的「权威」散落在另外 5 个文件里，两者之间没有任何绑定。**

- 改 YAML 节点名 → ① 断，无人知晓
- 改 YAML 的边 → ④ 断，无人知晓
- 改 agent 输出格式 → ② 断，无人知晓
- 回环进入下一轮 → ⑤ 不生效，agent 复述旧答案，无人知晓

流程只有「③ 决策」被当成了流程的一部分。它退化成了一台**流程图渲染器**：把节点连起来、按顺序调用 agent，但不感知、不判定、不设界、不重置记忆。真正在决定「下一步做什么」的，是那个永远返回 `false` 的默认值。

### 1.8 调度层为什么没发挥作用

```
scheduler-controller   每 5 秒轮询 task_executions.status
  → schedulerGraph.invoke
    → workerNode
      → graphRunner.startExecution
        → graph.invoke()      ← ①②③④⑤ 全部发生在这一个 await 的内部
          → 25 步（LangGraph 步数预算，非 agent 调用次数）
```

> **注意量级**：`Recursion limit of 25` 是 LangGraph 对 `graph.invoke()` 的**总步数**上限，不是"25 次 agent 调用"。本次执行实际只有 12 次 `final_output`。30 分钟来自"12 次执行 + 中间若干次空返"，不是"25 次调用"。设计预算时不要按 25 这个数字估算。

调度层只读得到 `task_executions.status`（pending / running / completed / failed）。它**分不清「卡住」和「正常跑」**（都是 `running`），没有任何流程级预算，也无法察觉流程在原地打转。

30 分钟里唯一报警的是 LangGraph 的通用步数上限 —— 那不是我们的流程在管，是框架在兜底，而且它数的是**步数**不是**代价**。
